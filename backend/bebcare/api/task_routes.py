...
@router.get("/drafts", response_model=List[ManualTaskDraftResponse])
def get_drafts(status: Optional[str] = "pending", db: Session = Depends(get_db)):
    query = db.query(ManualTaskDraft)
    if status:
        query = query.filter(ManualTaskDraft.status == status)
    drafts = query.order_by(ManualTaskDraft.created_at.desc()).all()
    result = []
    for draft in drafts:
        ...
        result.append({
            ...
            "published_platforms": draft.published_platforms or [],
