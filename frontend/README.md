...
需先启动后端（默认 `http://localhost:8888`，见 [backend/README.md](../backend/README.md)）。
...
开发时浏览器请求同源相对路径 `/v1`，由 Vite 代理到 `http://localhost:8888`（`vite.config.ts`），**无需**配置 `VITE_*` 后端地址。
