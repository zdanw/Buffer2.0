...
export const pagesEn: TranslationTree = {
  ...
  imageModelPicker: {
    ...
    tooltips: {
      title: 'Override the default image provider and model for this generation only.',
      provider: 'Image API provider. Leave empty to use your default provider.',
      modelId: 'Model or endpoint ID for the selected provider. Leave empty to use the provider default.',
...
export const pagesZh: TranslationTree = {
  ...
  imageModelPicker: {
    ...
    tooltips: {
      title: '仅本次生成覆盖默认图像提供商与模型。',
      provider: '图像 API 提供商。留空则使用你的默认供应商。',
      modelId: '所选提供商的模型或 Endpoint ID。留空则使用该提供商的默认模型。',
