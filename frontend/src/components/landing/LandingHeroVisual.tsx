...
export default function LandingHeroVisual() {
  ...
  return (
    <div className="relative w-full max-w-[520px] mx-auto aspect-[4/5] sm:aspect-square lg:aspect-[5/4]">
      <div className="absolute inset-0 rounded-3xl bg-forge-200/40 blur-[80px] animate-landing-glow pointer-events-none" />
      <div className="absolute top-[10%] -left-[8%] w-32 h-32 rounded-full bg-orange-100/80 blur-3xl animate-landing-float-slow pointer-events-none" />

      <div className="absolute top-[4%] left-[2%] z-20 animate-landing-float">
        <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-white/95 backdrop-blur border border-canvas-border shadow-card">
          <div className="w-8 h-8 rounded-lg bg-emerald-500 flex items-center justify-center">
            <PenLine className="w-4 h-4 text-white" strokeWidth={2} />
          </div>
          <div>
            <p className="text-[10px] text-ink-400 uppercase tracking-wider font-medium">
              {t('landing.previewCopy')}
            </p>
            <p className="text-xs text-ink-900 font-semibold">{t('landing.previewCopyHint')}</p>
          </div>
        </div>
      </div>

      <div className="absolute top-[18%] right-0 z-20 animate-landing-float-delay">
        <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-white/95 backdrop-blur border border-canvas-border shadow-card">
          <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-forge-400 to-forge-600 flex items-center justify-center">
            <Image className="w-4 h-4 text-white" strokeWidth={2} />
          </div>
          <div>
            <p className="text-[10px] text-ink-400 uppercase tracking-wider font-medium">
              {t('landing.previewImage')}
            </p>
            <p className="text-xs text-ink-900 font-semibold">{t('landing.previewImageHint')}</p>
          </div>
        </div>
      </div>

      <div className="absolute bottom-[22%] left-0 z-20 animate-landing-float-slow">
        <div className="px-3 py-2 rounded-xl bg-white/95 backdrop-blur border border-canvas-border shadow-card flex items-center gap-2">
          <Calendar className="w-4 h-4 text-forge-600" />
          <span className="text-xs text-ink-800 font-medium">{t('landing.heroChipSchedule')}</span>
        </div>
      </div>

      <div className="absolute inset-x-0 top-[12%] bottom-[8%] flex justify-center z-10 animate-landing-float-phone">
        <div className="relative w-[58%] max-w-[220px]">
          <div className="rounded-[32px] bg-gradient-to-b from-zinc-500 to-zinc-800 p-[7px] shadow-2xl shadow-ink-900/25 ring-1 ring-black/10">
