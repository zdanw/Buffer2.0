...
export default function Pagination({
  ...
  const handlePrev = () => {
    if (!disabled && currentPage > 1) onChange(currentPage - 1);
  };

  const handleNext = () => {
    if (!disabled && currentPage < pages) onChange(currentPage + 1);
  };

  const handleFirst = () => {
    if (!disabled && currentPage > 1) onChange(1);
  };
      ...
      <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
          ...
          {onPageSizeChange && (
            <div className="flex items-center gap-2">
              ...
              <select
                ...
                className="text-sm border border-gray-300 rounded-md px-2 py-1 bg-white focus:outline-none focus:ring-2 focus:ring-forge-500 disabled:opacity-50 disabled:cursor-not-allowed"
              >
        ...
        <div>
          <nav className="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
            ...
            <button onClick={handlePrev} disabled={navDisabled} className="relative inline-flex items-center px-2 py-2 border border-gray-300 bg-white text-sm font-medium text-gray-500 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed">
              ...
              typeof page === 'number' ? (
                <button
                  ...
                  className={`relative inline-flex items-center px-4 py-2 border text-sm font-medium disabled:cursor-not-allowed ${
                    page === currentPage
                      ? 'z-10 bg-forge-600 border-forge-600 text-white'
            ...
            <button onClick={handleNext} disabled={nextDisabled} className="relative inline-flex items-center px-2 py-2 border border-gray-300 bg-white text-sm font-medium text-gray-500 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed">
              <ChevronRight className="w-4 h-4" />
            </button>
            <button onClick={handleLast} disabled={nextDisabled} className="relative inline-flex items-center px-2 py-2 rounded-r-md border border-gray-300 bg-white text-sm font-medium text-gray-500 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed">
